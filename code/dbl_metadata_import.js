...
class DBLImport {
    ...
    addNamelike(domParent, jsonParent, children) {
        ...
        children.forEach((namelike, n) => {
            ...
            const namelikeLocalNode = self.childElementByName(
                domParent,
                namelike + "Local"
            );
            if (namelikeLocalNode) {
                namelikeJson[self.bcp47Local] =
                    namelikeLocalNode.childNodes[0].nodeValue;
            }
    ...
    processIdentification() {
        ...
        this.addNamelike(identification, this.sbMetadata.identification, [
            "name",
            "description",
            "abbreviation",
        ]);
    ...
    processRelationships() {
        ...
        const relationshipNodes = self.childElementsByName(
            relationships,
            "relation"
        );
        ...
        ["rightsHolder", "contributor", "rightsAdmin"].forEach((agencyType, n) => {
            if (agencyTypes[agencyType].length > 0) {
                for (var n = 0; n < agencyTypes[agencyType].length; n++) {
                    ...
                    const agencyUID = self.childElementByName(agency, "uid").childNodes[0]
                        .nodeValue;
                    var agencyJson;
    ...
    processType() {
        ...
        if (flavorName == "textTranslation") {
            ...
            typeJson.flavorType.flavor.audience = this.audience(
                audience.childNodes[0].nodeValue
            );
            const projectType = self.childElementByName(type, "projectType");
            if (projectType) {
                typeJson.flavorType.flavor.projectType = this.projectType(
                    projectType.childNodes[0].nodeValue
                );
            } else {
        ...
        const confidentialityJson = {
            ...
            ingredients: isConfidentialFlag ? "private" : "restricted",
        };
    ...
    processTypesetScriptureFormat(format, typeJson) {
        ...
        if (fontRecords.length > 0) {
            ...
            for (let n = 0; n < fontRecords.length; n++) {
                ...
                fontType =
                    fontType.substring(0, 1).toLowerCase() + fontType.substring(1);
    ...
    processAudioTranslationFormat(format, typeJson) {
        ...
        [
            ...
            if (fieldType == "integer") {
                typeJson.formats.mp3[field] = parseInt(
                    fieldNode.childNodes[0].nodeValue
                );
            } else if (fieldType == "ccString") {
                const fieldValue = fieldNode.childNodes[0].nodeValue;
                typeJson.formats.mp3[field] =
        ...
        [
            ...
            if (fieldType == "boolean") {
                typeJson.formats.format1.videoStream[field] =
                    fieldNode.childNodes[0].nodeValue == "true";
            } else if (fieldType == "integer") {
                typeJson.formats.format1.videoStream[field] = parseInt(
                    fieldNode.childNodes[0].nodeValue
                );
            } else if (fieldType == "ccString") {
                const fieldValue = fieldNode.childNodes[0].nodeValue;
    ...
    processEmbossedBrailleScriptureFormat(format, typeJson) {
        ...
        const numberSignCharacter = self.childElementByName(
            numberSign,
            "character"
        );
        ...
        typeJson.numberSign.useInMargin =
            numberSignMargin.childNodes[0].nodeValue == "true";
        typeJson.content = {};
        [
        ...
        const hyphenationDictionary = self.childElementByName(
            format,
        ...
        const continuousPoetry = self.childElementByName(
            format,
            "continuousPoetry"
        );
        if (continuousPoetry) {
    ...
    processCopyright() {
        ...
        const copyrightJson = {};
        const statementContents = self.childElementsByName(
            fullStatement,
            "statementContent"
        );
        if (statementContents.length > 0) {
            for (let n = 0; n < statementContents.length; n++) {
                ...
                if (contentType == "xhtml") {
                    ...
                    copyrightJson.fullStatementPlain[self.bcp47Local] =
                        statementContent.childNodes[0].nodeValue;
                }
            }
        }
    ...
    processPromotion() {
        ...
        const promotionJson = {};
        const promoVersionInfos = self.childElementsByName(
            promotion,
            "promoVersionInfo"
        );
        if (promoVersionInfos.length > 0) {
            for (let n = 0; n < promoVersionInfos.length; n++) {
                ...
                if (contentType == "xhtml") {
                    ...
                    promotionJson.statementPlain[self.bcp47Local] =
                        promoVersionInfo.childNodes[0].nodeValue;
                }
            }
        }
        ...
        */
        ...
        for (var n = 0; n < publication.length; n++) {
            ...
            for (var n2 = 0; n2 < contents.length; n2++) {
                ...
                if (
                    contentSrc in self.sbMetadata.ingredients &&
                    ...
                    if (truncatedRole.match("[A-Z0-6]{3}")) {
                        scope[role.substring(0, 3)] =
                            role.length == 3 ? [] : [role.substring(4)];
                        self.sbMetadata.ingredients[contentSrc].scope = scope;
                    } else {
                        self.sbMetadata.ingredients[contentSrc].role = role;
                    }
                    if (
                        truncatedRole.match("[A-Z0-6]{3}") &&
                        !(truncatedRole in currentScopeJson)
                    ) {
        ...
        if (Object.keys(currentScopeJson).length == 0) {
            for (var n = 0; n < publication.length; n++) {
                const publicationItem = publication.item(n);
                const canonical = self.childElementByName(
                    publicationItem,
                    "canonicalContent"
                );
        ...
        for (var n = 0; n < publication.length; n++) {
            ...
            const recipeSpecJson = {
                variantId: publicationItem.getAttribute("id"),
                processor: `https://thedigitalbiblelibrary.org/processors/${self.sbMetadata.type.flavorType.flavor.name}`,
                data: `entries/${self.sbMetadata.identification.systemId.dbl.id}/revisions/${self.sbMetadata.identification.systemId.dbl.revision}/publication/${publicationItem.getAttribute("id")}`,
            };
